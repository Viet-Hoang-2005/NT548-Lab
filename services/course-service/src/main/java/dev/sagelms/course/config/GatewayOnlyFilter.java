package dev.sagelms.course.config;
...
public class GatewayOnlyFilter extends OncePerRequestFilter {
    ...
    public GatewayOnlyFilter(@Value("${app.gateway.secret}") String gatewaySecret,
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (path.startsWith("/api/") && !gatewaySecret.equals(request.getHeader(GATEWAY_SECRET_HEADER))) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN, "Requests to course APIs must pass through gateway.");
            return;
        }
